...
function title() {
      console.clear()    
      console.log('\x1b[1;31m×\x1b[1;37m>', '<\x1b[1;32m📍︎\x1b[1;37m>', color('conecta a '), 'whatsaapweb')	
	  console.log(color(figlet.textSync(`${spc2}Itachi-MD`, {
        font: 'Standard',
	  ...
	  console.log(color(`•`, 'aqua'), color(`Holaaa          : Owner`, 'yellow'))
	  console.log(color(`•`, 'aqua'), color(`Mi versión : 1.0.0`, 'lime'))
	  console.log(color(`•`, 'aqua'), color(`Estado       : Online!`, 'white'))
	  console.log(color(`•`, 'aqua'), color(`Onwer       : Edwin`, 'red'))
	  console.log(color(`•`, 'aqua'), color(`Developer    : Antonio`, 'magenta'))
}
// COLOR
colors.setTheme({
warn:'yellow',
    ...
    conn.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                let link = `https://instagram.com/ant0dwn`
let buttons = [
...
conn.sendMessage(anu.id, buttonMessage, {quoted:fgclink})
...
conn.sendMessage(anu.id, buttonMessage, {quoted:fgclink})
                    ...
                    conn.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promovido en ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    conn.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Degradado en ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await conn.getName(i + '@s.whatsapp.net')}\nFN:${await conn.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Agg Antonio👀\nitem2.EMAIL;type=INTERNET: antonioaguilar626212@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/ant0dwn?igshid=YmMyMTA2M2Y=\nitem3.X-ABLabel:Instagram\nitem4.ADR:;; Panamá 🇵🇦;;;;\nitem4.X-ABLabel:Region\nitem5.OTRO;type=simple:By Antonio\nitem5.X-ABLabel:Tópico\nEND:VCARD`
	    })
	}
